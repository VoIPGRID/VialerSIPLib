...
/**
 Creates and initializes an NSError object for a given domain and code. It will also make an userInfo dictionary base on the param that are given (underlyingErrorKey, localizedDescriptionKey and localizedFailureReasonError).
 ...
 */
+ (NSError * _Nonnull)VSLUnderlyingError:(NSError * _Nullable)underlyingErrorKey localizedDescriptionKey:(NSString * _Nullable)localizedDescriptionKey localizedFailureReasonError:(NSString * _Nullable)localizedFailureReasonError errorDomain:(NSString * _Nonnull)errorDomain errorCode:(NSUInteger)errorCode;
